package me.kxda.hermes.utils.finalutil;
...
public final class PlayerUtil
{
    ...
    public static void messageStaff(final String message, final Rank rank) {
        Bukkit.getOnlinePlayers().stream().filter( player -> Hermes.getInstance().getPlayerManager().getPlayer(player.getUniqueId()).hasRank(rank)).forEach(player -> player.sendMessage(message));
    }

    public static boolean testPermission(final CommandSender sender, final Rank requiredRank) {
        if (sender instanceof Player) {
            final Profile profile = Hermes.getInstance().getPlayerManager().getPlayer(((Player)sender).getUniqueId());
    ...
    static {
        ...
        VISIBLE_RANK_ORDER = ((a, b) -> {
            profileA[0] = Hermes.getInstance().getPlayerManager().getPlayer(a.getUniqueId());
            profileB[0] = Hermes.getInstance().getPlayerManager().getPlayer(b.getUniqueId());
